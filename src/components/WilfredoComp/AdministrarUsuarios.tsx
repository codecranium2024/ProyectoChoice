...
const AdministrarUsuarios: React.FC = () => {
  ...
  // Función para manejar la generación de usuario y contraseña automáticos
  const generateUserAndPassword = () => {
    const { Nombre, Apellido, Telefono } = registerForm;
    const apellidoPrimerParte = Apellido.split(' ')[0]; // Tomar solo el primer apellido
    // Generar nombre de usuario
    const user = `${Nombre.slice(0, 2).toLowerCase()}${apellidoPrimerParte.toLowerCase()}${Telefono.slice(-3)}`;
    ...
    // Actualizar el estado y devolver los valores generados
    setAutoGeneratedUser(user);
  ...
  const handleRegisterUser = async () => {
    ...
    if (!customPassword) {
      const generated = generateUserAndPassword();
      usuarioGenerado = generated.user;
      passwordGenerado = generated.password;
    }
